  ...
  for (let g = 0; g < model.ngeom; g++) {
    ...
    if (type == mujoco.mjtGeom.mjGEOM_PLANE.value) {
      ...
      if (!(meshID in meshes)) {
        ...
        let swizzled_uv_buffer = new Float32Array(
          (vertex_buffer.length / 3) * 2,
        );
        ...
        geometry.setAttribute(
          "normal",
          new THREE.BufferAttribute(swizzled_normal_buffer, 3),
        );
        geometry.setAttribute(
          "uv",
          new THREE.BufferAttribute(swizzled_uv_buffer, 2),
        );
    ...
    if (model.geom_matid[g] != -1) {
      ...
      if (texId != -1) {
        ...
        for (let p = 0; p < width * height; p++) {
          ...
          rgbaArray[p * 4 + 1] =
            channels > 1
              ? texData[offset + (p * channels + 1)]
              : rgbaArray[p * 4 + 0];
          rgbaArray[p * 4 + 2] =
            channels > 2
              ? texData[offset + (p * channels + 2)]
              : rgbaArray[p * 4 + 0];
          rgbaArray[p * 4 + 3] =
            channels > 3 ? texData[offset + (p * channels + 3)] : 255;
        }
  ...
  if (mujocoRoot && mujocoRoot.cylinders) {
    ...
    for (let t = 0; t < model.ntendon; t++) {
      ...
      for (let w = startW; w < startW + data.ten_wrapnum[t] - 1; w++) {
        ...
        if (validStart) {
          mujocoRoot.spheres.setMatrixAt(
            numWraps,
            mat.compose(tendonStart, identityQuat, new THREE.Vector3(r, r, r)),
          );
        }
        if (validEnd) {
          mujocoRoot.spheres.setMatrixAt(
            numWraps + 1,
            mat.compose(tendonEnd, identityQuat, new THREE.Vector3(r, r, r)),
          );
        }
